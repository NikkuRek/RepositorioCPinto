...
function Header()
{
    ...
    $this->Cell(1,10,utf8_decode('Lista de entradas'),0,1,'C',0);
    $this->Ln(20);
}
...
// Llamada al modelo para obtener datos de entradas
$reporte = new ReportesModelo();
$entradas = $reporte->obtenerDatosEntradas();
...
$pdf->Cell(60,10,utf8_decode('FECHA DE ENTRADA'),1,1,'C',1);
...
for($i=0;$i<count($entradas);$i++){
    ...
    $pdf->Cell(20,10,$entradas[$i]['codigo_entrada'],1,0,'C',1);
    $pdf->Cell(55,10,$entradas[$i]['producto'],1,0,'C',1);
	$pdf->SetFont('Arial','',8);
    $pdf->Cell(35,10,$entradas[$i]['cantidad_entrada'],1,0,'C',1);
    $pdf->Cell(60,10,$entradas[$i]['fecha_entrada'],1,0,'C',1);
	$pdf->Ln(10);
}
$pdf->Output('I','Entradas'.'.pdf');  // envia el documento
?>