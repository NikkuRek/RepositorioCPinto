...
function Header()
{
    ...
    $this->Cell(100,25,utf8_decode('Pedidos Cerrados'),0,1,'C',0);
    $this->Ln(20);
}
...
if (isset($_GET['fechai']) && isset($_GET['fechaf'])) {
    ...
    // Llamada al modelo para obtener datos de pcer$pcerrados
    $reporte = new ReportesModelo();
    $reporte->set_fechai($fechai);
    $reporte->set_fechaf($fechaf);
    $pcerrados= $reporte->obtenerDatosPCerrados();
    ...
    $pdf->Cell(35,9,utf8_decode('FECHA ENTREGA'),1,0,'C',1);
    $pdf->Cell(70,9,utf8_decode('NOMBRE DE PEDIDO'),1,0,'C',1);
    $pdf->Cell(40,9,utf8_decode('NOMBRE CLIENTE'),1,0,'C',1);
    $pdf->Cell(30,9,utf8_decode('ESTATUS'),1,1,'C',1);
    ...
    for($i = 0; $i < count($pcerrados); $i++) {
        ...
        $pdf->Cell(35, 7, utf8_decode($pcerrados[$i]['fecha']), 1, 0, 'C', 1);
        $pdf->Cell(70, 7, utf8_decode($pcerrados[$i]['nombre_pedido']), 1, 0, 'C', 1);
        $pdf->Cell(40, 7, utf8_decode($pcerrados[$i]['nombre_cliente']), 1, 0, 'C', 1);
        $pdf->Cell(30, 7, utf8_decode($pcerrados[$i]['estatus']), 1, 0, 'C', 1);
        $pdf->Ln(7);
    }
   ...
   // $pdf->Cell(190, 9, utf8_decode('Total General: '.number_format($total, 2)), 1, 0, 'C', 1);
    ...
    $pdf->Output('I', 'pcer$pcerrados.pdf');
} else {
