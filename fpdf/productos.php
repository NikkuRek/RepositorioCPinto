...
function Header()
{
    ...
    $this->Cell(1,10,utf8_decode('Lista de productos'),0,1,'C',0);
    $this->Ln(20);
}
...
// Llamada al modelo para obtener datos de productos
$reporte = new ReportesModelo();
$productos = $reporte->obtenerDatosProductos();
...
$pdf->Cell(30,10,utf8_decode('DESCRIP.'),1,0,'C',1);
$pdf->Cell(30,10,utf8_decode('PRECIO($)'),1,0,'C',1);
$pdf->Cell(35,10,utf8_decode('CANTIDAD'),1,0,'C',1);
$pdf->Cell(35,10,utf8_decode('PROVEEDOR'),1,1,'C',1);
...
for($i=0;$i<count($productos);$i++){
    ...
    $pdf->Cell(20,10,$productos[$i]['id_producto'],1,0,'C',1);
    $pdf->Cell(30,10,$productos[$i]['nombre_prod'],1,0,'C',1);
    $pdf->Cell(30,10,$productos[$i]['descripcion'],1,0,'C',1);
    $pdf->Cell(30,10,$productos[$i]['precio'],1,0,'C',1);
	$pdf->SetFont('Arial','',8);
    $pdf->Cell(35,10,$productos[$i]['cantidad_disp'],1,0,'C',1);
    $pdf->Cell(35,10,$productos[$i]['proveedor'],1,0,'C',1);
	$pdf->Ln(10);
}
$pdf->Output('I','Productos'.'.pdf');  // envia el documento
?>