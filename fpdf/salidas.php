...
function Header()
{
    ...
    $this->Cell(1,10,utf8_decode('Lista de salidas'),0,1,'C',0);
    $this->Ln(20);
}
...
// Llamada al modelo para obtener datos de salidas
$reporte = new ReportesModelo();
$salidas = $reporte->obtenerDatossalidas();
...
$pdf->Cell(60,10,utf8_decode('FECHA DE SALIDA'),1,1,'C',1);
...
for($i=0;$i<count($salidas);$i++){
    ...
    $pdf->Cell(20,10,$salidas[$i]['codigo_salida'],1,0,'C',1);
    $pdf->Cell(55,10,$salidas[$i]['producto'],1,0,'C',1);
	$pdf->SetFont('Arial','',8);
    $pdf->Cell(35,10,$salidas[$i]['cant_salida'],1,0,'C',1);
    $pdf->Cell(60,10,$salidas[$i]['fecha_salida'],1,0,'C',1);
	$pdf->Ln(10);
}
$pdf->Output('I','salidas'.'.pdf');  // envia el documento
?>